#!/usr/bin/env bash
# Fine-tune Qwen3-VL-2B on real-robot cogwam data (RobotWin-compatible format).
#
# Default data: /media/damoxing/datasets/VLN-CE/cogwam_data/20260629
#
# Optional warm-start from sim RobotWin checkpoint:
#   ROBOTWIN_INIT_CHECKPOINT=/media/damoxing/ckp/qwen_ft/robotwin_qwen3vl_2b/pytorch_model.bin \
#   bash qwen-vl-finetune/scripts/sft_robotwin_qwen3vl_2b_real.sh
#
...
#   bash qwen-vl-finetune/scripts/sft_robotwin_qwen3vl_2b_real.sh
#
# Resume from a previous real-data run (auto-detected if checkpoint exists in OUTPUT_DIR):
#   bash qwen-vl-finetune/scripts/sft_robotwin_qwen3vl_2b_real.sh
set -euo pipefail
