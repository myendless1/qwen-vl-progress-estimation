        ...
        if next_close is not None and next_close < next_open:
            ...
            if carrier_open is not None:
                steps.append(
                    StepSpec(
                        (
                            f"Open the gripper of the {carrier} arm while moving the {receiver} arm "
                            f"to place the {obj} into the dustbin, resting on the bottom."
                        ),
                        "handover",
                        receiver,
                    )
                ...
                steps.append(
                    StepSpec(
                        f"Move the {receiver} arm to place the {obj} into the dustbin, resting on the bottom.",
                        "move",
                        receiver,
                    )
                )
...
TASK_BUILDERS["place_cans_plasticbox"] = dual_container_builder(
    "left can", "right can", "plastic box", "object_b",
...
TASK_BUILDERS["place_dual_shoes"] = dual_container_builder(
    "left shoe", "right shoe", "shoe box", "object_b",
    lift_between_releases=False,
...
def handover_block(ctx: TaskContext) -> list[StepSpec]:
    ...
    return [
        ...
        StepSpec("Move to the grasp pose of the red block for handover.", "move", receiving_arm),
        StepSpec(close_gripper_text(receiving_arm), "close", receiving_arm),
        StepSpec(
            f"Open the gripper of the {first_open_arm} arm while moving the {receiving_arm} arm to take over the red block.",
            "handover",
            receiving_arm,
        ),
        StepSpec(
...
def handover_mic(ctx: TaskContext) -> list[StepSpec]:
    ...
    return [
        ...
        StepSpec(f"Move to the grasp pose of the {obj} for handover.", "move", receiving_arm),
        StepSpec(close_gripper_text(receiving_arm), "close", receiving_arm),
        StepSpec(
            f"Open the gripper of the {release_arm} arm while moving the {receiving_arm} arm to take over the {obj}.",
            "handover",
            receiving_arm,
        ),
    ]
...
def rotate_qrcode(ctx: TaskContext) -> list[StepSpec]:
    ...
    return grasp_steps(obj, ctx.arm_a) + [
        StepSpec(f"Rotate the {obj} until the QR code faces the robot and put down the QR code.", "move", ctx.arm_a),
        StepSpec(open_gripper_text(ctx.arm_a), "open", ctx.arm_a),
    ]
...
def open_microwave(ctx: TaskContext) -> list[StepSpec]:
    ...
    final_text = (
        f"Open the {obj} {part} to at least 63 degrees."
        if len(close_events) <= 1
        else f"Continue opening the {obj} {part} to at least 63 degrees."
    )
...
def single_press_builder(text: str | Callable[[TaskContext], str]) -> TaskBuilder:
    def builder(ctx: TaskContext) -> list[StepSpec]:
        ...
        target = "alarm clock button" if "alarm clock" in value.lower() else "bell"
        return [
            StepSpec(
                (
                    f"Move the {ctx.arm_a} arm above the {target} and close the gripper."
                    if ctx.arm_a
...
def press_stapler(ctx: TaskContext) -> list[StepSpec]:
    return [
        StepSpec(
            f"Move the {ctx.arm_a} arm above the stapler and close the gripper."
            if ctx.arm_a
